...
class MerakiCreateGoldenNetwork(MerakiHandler):
    ...
    def update_network_settings(self, new_network):
        try:
            ...
            response = self.dashboard.networks.updateNetworkAlertsSettings(new_network['id'], 
                defaultDestinations={'emails': ['kcc-infrastructure-alerts@kccmfg.com'], 'allAdmins': True, 'snmp': False, 'httpServerIds': [cbts_listener_uat['id'], cbts_listener['id']]}, 
    ...
    # 4. Wirelsess -> SSIDs
    def create_wireless_ssid(self, new_network, ssids_data):
        try: 
    ...
    def updateNetworkWirelessSettings(self, new_network, wirelesssettings):
        try: 
            response = self.dashboard.wireless.updateNetworkWirelessSettings(new_network['id'], **wirelesssettings)
            print(response)
            return "Successfully updated NetworkWirelessSettings"
        except Exception as error:
            print("Error occurred while updateNetworkWirelessSettings: {}".format(error))
            self.delete_new_network(new_network)

    def updateNetworkWirelessSsidTrafficShapingRules(self, new_network, ssid, wirelessSsidTrafficShapingRules):
        try: 
            response = self.dashboard.wireless.updateNetworkWirelessSsidTrafficShapingRules(new_network['id'], ssid['number'], **wirelessSsidTrafficShapingRules)
            print(response)
            return "Successfully updated updateNetworkWirelessSsidTrafficShapingRules"
        except Exception as error:
            print("Error occurred while updateNetworkWirelessSsidTrafficShapingRules: {}".format(error))
            self.delete_new_network(new_network)
    ...
    def create_group_policies(self, new_network, group_policies):
        try:
    ...
    def update_device(self, serial, device):
        try:
            new_device = {i:device[i] for i in device if i!='networkId' and i!='serial' and i!='url' and i!='switchProfileId' and i!='firmware'}
            # print(new_device)
            print("Updating: serial {}, name {}".format(serial, device['name']))
            # print(new_device)
            response = self.dashboard.devices.updateDevice(serial, **new_device)
            return response
            # return 0
        except Exception as error:
            print("Error occurred update_devices: {}".format(error))

    def createNetworkSwitchStack(self, new_network, switch_stack):
        try:
            new_switch_stack = {i:switch_stack[i] for i in switch_stack if i!='id'}
            response = self.dashboard.switch.createNetworkSwitchStack(new_network['id'], **new_switch_stack)
            return response
        except Exception as error:
            print("Error occurred createNetworkSwitchStack: {}".format(error))

    def createNetworkSwitchLinkAggregation(self, new_network, switchPorts):
        try:
            print(switchPorts)
            # response = self.dashboard.switch.createNetworkSwitchLinkAggregation(new_network['id'], switchPorts)
            # return response
            return 0
        except Exception as error:
            print("Error occurred createNetworkSwitchLinkAggregation: {}".format(error))
